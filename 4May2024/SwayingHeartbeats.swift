...
struct SwayingHeartbeats: View {
    ...
    var SwayingHeartbeats = "SwayingHeartbeats"
    func playAllSongs() {
                // Iterate over each song in the array and play audio
        
                    if let url = Bundle.main.url(forResource: SwayingHeartbeats, withExtension: "mp3") {
                        do {
        ...
        ZStack{
            ...
            HStack{
                Button{
                    isPlayingMusic = true
                    playAllSongs()
                    savedSound = SwayingHeartbeats
                    progress -= progress
                    progress2 = 0.003
                }label:{
                    Text(SwayingHeartbeats)
                        .foregroundColor(.red)
                        .font(.title)
                        .padding()
                }
