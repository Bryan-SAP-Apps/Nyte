...
let musicPlayerAtmosphere = MusicPlayerAtmosphere()
struct AtmosphericNoiseView: View {
    ...
    static var songs = [Music(title:"dutchcountryside", artist: "Samuel"),Music(title: "night-at-campfire", artist: "Samuel"), Music(title: "NocturnalReverie", artist: "Samuel")]
    ...
    func playNextSong() {
        let nextIndex = (currentIndex + 1) % AtmosphericNoiseView.songs.count
        
        let song = AtmosphericNoiseView.songs[nextIndex]
    ...
    func playPreviousSong() {
            let previousIndex = (currentIndex - 1 + AtmosphericNoiseView.songs.count) % AtmosphericNoiseView.songs.count
            
            let song = AtmosphericNoiseView.songs[previousIndex]
    ...
    func loopSongs() {
        ...
        while true {
            let song = AtmosphericNoiseView.songs[currentIndex]
            ...
            if let url = Bundle.main.url(forResource: song.title, withExtension: "mp3") {
                do {
                    ...
                    currentIndex = (currentIndex + 1) % AtmosphericNoiseView.songs.count
                } catch {
                    print("Error playing audio for \(song.title): \(error.localizedDescription)")
                }
            } else {
    ...
    func startShufflePlay() {
        ...
        shuffledSongs = AtmosphericNoiseView.songs.shuffled()
    ...
    class Coordinator: NSObject, AVAudioPlayerDelegate {
        var view: AtmosphericNoiseView
        
        init(view: AtmosphericNoiseView) {
            self.view = view
        }
...
//
                                ...
                                Button{
                                    ...
                                    progress -= progress
                                    progress2 = 0.003
                                    musicPlayerAtmosphere.loopSongs()
                                }label:{
